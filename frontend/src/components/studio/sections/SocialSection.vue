...
import { socialPostsAPI } from '@/lib/api/studio'
...
const { savedJobs, currentJob, isGenerating, loadSavedJobs, handleGeneration } =
  useStudioGeneration({
    ...
    loadJobs: async () => {
      const res = await socialPostsAPI.listJobs(projectId)
    ...
    startGeneration: async (signal: StudioSignal) => {
      ...
      return socialPostsAPI.startGeneration(projectId, sourceId, signal.direction)
    },
    pollJobStatus: (jobId: string, onProgress: any) => socialPostsAPI.pollJobStatus(projectId, jobId, onProgress),
  })

const filteredJobs = useFilteredJobs(savedJobs)

onMounted(loadSavedJobs)
registerGenerationHandler('social', handleGeneration)
</script>
...
<template>
  <template v-if="filteredJobs.length > 0 || isGenerating">
    <GenericProgressIndicator
      v-if="isGenerating"
      :title="currentJob?.source_name || 'Generating Social Posts...'"
    ...
    <GenericListItem
      ...
      :key="job.id"
      :title="(job as any).title || (job as any).source_name || 'Social Posts'"
      :icon="PhShareNetwork"
      icon-bg-class="bg-cyan-500/10"
