...
import { flashCardsAPI } from '@/lib/api/studio'
...
const { savedJobs, currentJob, isGenerating, loadSavedJobs, handleGeneration } =
  useStudioGeneration({
    itemLabel: 'Flash Cards',
    loadJobs: async () => {
      const res = await flashCardsAPI.listJobs(projectId)
    ...
    startGeneration: async (signal: StudioSignal) => {
      ...
      return flashCardsAPI.startGeneration(projectId, sourceId, signal.direction)
    },
    pollJobStatus: (jobId: string, onProgress: any) => flashCardsAPI.pollJobStatus(projectId, jobId, onProgress),
  })

const filteredJobs = useFilteredJobs(savedJobs)

onMounted(loadSavedJobs)
registerGenerationHandler('flash_cards', handleGeneration)
</script>
...
<template>
  <template v-if="filteredJobs.length > 0 || isGenerating">
    <GenericProgressIndicator
      v-if="isGenerating"
      :title="currentJob?.source_name || 'Generating Flash Cards...'"
    ...
    <GenericListItem
      ...
      :key="job.id"
      :title="(job as any).title || (job as any).source_name || 'Flash Cards'"
      :icon="PhCards"
      icon-bg-class="bg-purple-500/10"
