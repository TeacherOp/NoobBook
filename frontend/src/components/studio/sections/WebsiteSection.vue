...
import { websitesAPI } from '@/lib/api/studio'
...
const { savedJobs, currentJob, isGenerating, loadSavedJobs, handleGeneration } =
  useStudioGeneration({
    itemLabel: 'Website',
    loadJobs: async () => {
      const res = await websitesAPI.listJobs(projectId)
    ...
    startGeneration: async (signal: StudioSignal) => {
      ...
      return websitesAPI.startGeneration(projectId, sourceId, signal.direction)
    },
    pollJobStatus: (jobId: string, onProgress: any) => websitesAPI.pollJobStatus(projectId, jobId, onProgress),
  })

const filteredJobs = useFilteredJobs(savedJobs)

onMounted(loadSavedJobs)
registerGenerationHandler('website', handleGeneration)
</script>
...
<template>
  <template v-if="filteredJobs.length > 0 || isGenerating">
    <GenericProgressIndicator
      v-if="isGenerating"
      :title="currentJob?.source_name || 'Generating Website...'"
    ...
    <GenericListItem
      ...
      :key="job.id"
      :title="(job as any).title || (job as any).source_name || 'Website'"
      :icon="PhGlobe"
      icon-bg-class="bg-sky-500/10"
      icon-color-class="text-sky-600"
      @open="() => {}"
      @download="() => {}"
    />
  </template>
